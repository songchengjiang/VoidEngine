...
void caculateShadowTextureCoord(in vec4 position)
{
	...
	for (int i = 0; i < VE_DIRECTIONAL_LIGHT_MAX_NUM; ++i){
		if (0 < ve_dirLightVisible[i] && 0 < ve_dirLightShadowEnabled[i]){
	...
	for (int i = 0; i < VE_POINT_LIGHT_MAX_NUM; ++i){
		if (0 < ve_pointLightVisible[i] && 0 < ve_pointLightShadowEnabled[i]){
	...
	for (int i = 0; i < VE_SPOT_LIGHT_MAX_NUM; ++i){
		if (0 < ve_spotLightVisible[i] && 0 < ve_spotLightShadowEnabled[i]){
