	...
	int interpolationPasses, float interpolationThreshold)
{
	...
	int w[] = {
		static_cast<int>(ctx->hemisphere.fbHemiCountX * ctx->hemisphere.size * 3),
		static_cast<int>(ctx->hemisphere.fbHemiCountX * ctx->hemisphere.size / 2) };
	int h[] = {
		static_cast<int>(ctx->hemisphere.fbHemiCountY * ctx->hemisphere.size),
		static_cast<int>(ctx->hemisphere.fbHemiCountY * ctx->hemisphere.size / 2) };
...
lm_bool lmImageSaveTGAub(const char *filename, const unsigned char *image, int w, int h, int c)
{
	...
	unsigned char header[18] = { 0, 0, static_cast<unsigned char>(isGreyscale ? 3 : 2), 0, 0, 0, 0, 0, 0, 0, 0, 0, static_cast<unsigned char>(w & 0xff), static_cast<unsigned char>((w >> 8) & 0xff), static_cast<unsigned char>(h & 0xff), static_cast<unsigned char>((h >> 8) & 0xff), (unsigned char)(8 * c), static_cast<unsigned char>(hasAlpha ? 8 : 0) };
