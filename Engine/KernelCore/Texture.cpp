		...
		if (_manager->exchangeTextureMemory(this, contextID)) {
			...
			if (_isCompressedTex) {
				if (_data) {
					...
					glCompressedTexImage2D(_target, 0, _internalFormat, _width, _height, 0, imageSize, _data);
				}
					...
					for (size_t lev = 0; lev < _mipmapLevels.size(); ++lev) {
						...
						glCompressedTexImage2D(_target, lev, _internalFormat, _mipmapLevels[lev].width, _mipmapLevels[lev].height, 0, imageSize, _mipmapLevels[lev].data);
					} 
				}
			}
			else {
				if (_data) {
					glTexImage2D(_target, 0, _internalFormat, _width, _height, 0, _pixelFormat, _dataType, _data);
					generateMipMaps();
				}
				else if (!_mipmapLevels.empty()) {
					for (size_t lev = 0; lev < _mipmapLevels.size(); ++lev) {
						glTexImage2D(_target, lev, _internalFormat, _mipmapLevels[lev].width, _mipmapLevels[lev].height, 0, _pixelFormat, _dataType, _mipmapLevels[lev].data);
                ...
                for (unsigned int i = 0; i < 6; ++i) {
                    ...
                    if (_isCompressedTex) {
                        if (tex->getData()) {
                            ...
                            glCompressedTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, _internalFormat, tex->getWidth(), tex->getHeight(), 0, imageSize, tex->getData());
                        }
                            ...
                            for (size_t lev = 0; lev < mipmaps.size(); ++lev) {
                                ...
                                glCompressedTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, lev, _internalFormat, mipmaps[lev].width, mipmaps[lev].height, 0, imageSize, mipmaps[lev].data);
                            }
                        }
                    }
                    else {
                        if (tex->getData()) {
                            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, _internalFormat, tex->getWidth(), tex->getHeight(), 0, tex->getPixelFormat(), tex->getDataType(), tex->getData());
                            generateMipMaps();
                        }
                        else if (!tex->getMipmapLevels().empty()) {
                            const auto &mipmaps = tex->getMipmapLevels();
                            for (size_t lev = 0; lev < mipmaps.size(); ++lev) {
                                glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, lev, _internalFormat, mipmaps[lev].width, mipmaps[lev].height, 0, tex->getPixelFormat(), tex->getDataType(), mipmaps[lev].data);
                            }
                            generateMipMaps();
                        }
                    }
